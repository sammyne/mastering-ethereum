...
func main() {
	...
	calledContractCode, calledLibraryCode, err := generateBinaryABI("CallExamples.sol")
	if err != nil {
		panic(err)
	}
	...
	{
		fmt.Println("deploying called contract...")

		tx := types.NewContractCreation(nonce, toWei(0), gasLimit, gasPrice, calledContractCode)
		...
		fmt.Println("done deploying called contract")
	}

	{
		fmt.Println("deploying called library...")

		tx := types.NewContractCreation(nonce+1, toWei(0), gasLimit, gasPrice, calledLibraryCode)
		if tx, err = store.SignTx(sender, tx, chainID); nil != err {
			panic(err)
		}
		...
		fmt.Println("done deploying called library")
	}
}

func generateBinaryABI(contract string) ([]byte, []byte, error) {
	workingDir, err := os.Getwd()
	if err != nil {
		return nil, nil, fmt.Errorf("fail to get current working dir: %w", err)
	}
	...
	if err := cmd.Run(); err != nil {
		return nil, nil, fmt.Errorf("fail to run cmd: %w", err)
	}
	...
	var (
		calledContractCodeHex string
		calledLibraryCodeHex  string
	)
	for i, v := range raw {
		switch {
		case strings.Contains(v, "calledContract ======="):
			calledContractCodeHex = raw[i+2]
		case strings.Contains(v, "calledLibrary ======="):
			calledLibraryCodeHex = raw[i+2]
		default:
		}
	}

	if calledContractCodeHex == "" || calledLibraryCodeHex == "" {
		return nil, nil, fmt.Errorf("fail to read code for calledContract or calledLibrary")
	}

	calledContractCode, err := hex.DecodeString(calledContractCodeHex)
	if err != nil {
		return nil, nil, fmt.Errorf("fail to decode ABI code for calledContract: %w", err)
	}

	calledLibraryCode, err := hex.DecodeString(calledLibraryCodeHex)
	if err != nil {
		return nil, nil, fmt.Errorf("fail to decode ABI code for calledContract: %w", err)
	}

	return calledContractCode, calledLibraryCode, nil
}
