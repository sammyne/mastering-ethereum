...
func main() {
	...
	withdrawSig = append(withdrawSig, math.PaddedBigBytes(toWei(0.01), 32)...)

	// withdraw function cannot receive ether, so amount must be 0
	tx := types.NewTransaction(nonce, receipt.ContractAddress, toWei(0), gasLimit, gasPrice,
		withdrawSig)
